...
- [x] 실행 관리 페이지 라우트 설정 (/executions, /executions/new, /executions/[id])
- [x] API 클라이언트 (executionApi.ts) 생성
- [x] 공통 훅 (useExecutions, useExecution, useCreateExecution, useCancelExecution) 구조 생성
- [x] 타입 정의 및 공통 유틸리티 설정
...
1. [x] 모든 라우트가 접근 가능 (404 없음)
2. [x] API 클라이언트가 백엔드와 통신 가능
3. [x] React Query 훅이 정상 동작
4. [x] 사이드바에 실행 메뉴가 표시됨
5. [x] TypeScript 컴파일 성공
6. [x] GitHub Actions CI 통과 (로컬 빌드 성공)

## 구현 결과

### 생성된 파일
- `packages/frontend/src/services/executionApi.ts` - API 클라이언트
...
- `packages/frontend/src/hooks/executions/index.ts` - 훅 export
- `packages/frontend/src/app/executions/page.tsx` - 목록 페이지
- `packages/frontend/src/app/executions/new/page.tsx` - 생성 페이지
- `packages/frontend/src/app/executions/[id]/page.tsx` - 상세 페이지

### 수정된 파일
- `packages/frontend/src/components/Providers.tsx` - QueryClientProvider 추가
