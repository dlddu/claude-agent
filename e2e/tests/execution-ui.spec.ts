...
test.describe('Execution Pages - Auth Protection', () => {
  test('execution list page should redirect to login', async ({ page }) => {
    await page.goto('/executions');
    await page.waitForURL(/\/login/, { timeout: 10000 });
    expect(page.url()).toContain('/login');
  });

  test('new execution page should redirect to login', async ({ page }) => {
    await page.goto('/executions/new');
    await page.waitForURL(/\/login/, { timeout: 10000 });
    expect(page.url()).toContain('/login');
  });

  test('execution detail page should redirect to login', async ({ page }) => {
